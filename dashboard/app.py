...
def load_annual_accumulated():
    ...
    if path.exists():
        df = pd.read_csv(path)
        df.rename(
            columns={"year": "year", "inflacion_mensual": "inflacion_anual"},
            inplace=True,
        )
        return df
    return None
...
if df_anual is None:
    st.error(
        "No se encontraron datos acumulados anuales. Ejecuta 'python scripts/run_analysis_monthly.py'"
    )
    st.stop()
...
tab1, tab2, tab3 = st.tabs(
    [
        "📈 Inflación Anual (BCRA acumulado)",
...
with tab1:
    ...
    df_filtrado = df_anual[
        (df_anual["year"] >= year_range[0]) & (df_anual["year"] <= year_range[1])
    ]
    chart_type = st.radio(
        "Tipo de gráfico",
        ["Línea", "Barras", "Ambos"],
    ...
    ax.set_title(
        f"Evolución de la inflación anual acumulada ({year_range[0]}-{year_range[1]})"
    )
...
with tab2:
    if df_mensual is None:
        ...
        df_filtrado_m = df_mensual[
            (df_mensual["date"].dt.date >= date_range[0])
            & (df_mensual["date"].dt.date <= date_range[1])
        ]
        chart_type_m = st.radio(
            "Tipo de gráfico",
            ["Línea", "Barras", "Ambos"],
            horizontal=True,
            key="mensual_chart",
        ...
        if chart_type_m in ["Línea", "Ambos"]:
            ax2.plot(
                df_filtrado_m["date"],
                df_filtrado_m["inflacion_mensual"],
                marker=".",
                color="#1f77b4",
        ...
        col1.metric(
            "Último mes",
            ultimo_mes["date"].strftime("%Y-%m"),
            f"{ultimo_mes['inflacion_mensual']:.2f}%",
        )
...
with tab3:
    ...
    st.info(
        "La inflación anual acumulada se calcula a partir de los datos mensuales del BCRA."
    )
